...
class Room(DefaultRoom):
    ...
    def detail_string(self, target, looker):
        ...
        if target.has_account:
            ...
            if target.locks.check_lockstring(target, "dummy:perm(builder)"):
                marker = "|540*|n "
            elif target.locks.check_lockstring(target, "dummy:perm(helper)"):
                marker = "|113!|n "
            else:
                marker = "  "
    ...
    def return_appearance(self, looker):
        ...
        for con in visible:
            if con.destination:
                exits[con.destination.db.type].append(con)
            elif con.has_account:
                users.append(con)
            else:
                things[con.key].append(con)
